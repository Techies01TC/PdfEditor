...
const PDFViewerApplication = {
  ...
  async download(options = {}) {
    ...
    try {
      ...
      //window.ReactNativeWebView.postMessage(JSON.stringify({ type: "startDownload"}));
      ///////////////////////////////////////////
      PDFViewerApplication.toolbar?.updateLoadingIndicatorState(true);      
      PDFViewerApplication.pdfDocumentProperties?.open();
      // Get the button element by its id
      var closeButton = document.getElementById("documentPropertiesClose");

      // Change the text inside the button
      closeButton.innerHTML = "Saving, Please Wait....";
      ...
      // window.ReactNativeWebView.postMessage(JSON.stringify({ type: "blobUrl", blobUrl: blobUrl, filename: '111111'+filename }));

      
      ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      // Assuming you have the Blob data
      const blob = new Blob([data], { type: "application/pdf" });
      ...
      // Once the reader has loaded the data, handle it
      reader.onload = () => {
        // Get the data URL
        const dataUrl = reader.result;

        window.ReactNativeWebView.postMessage(JSON.stringify({ type: "blobUrl", dataUrl: dataUrl, filename: filename }));
      ...
      // window.ReactNativeWebView.postMessage(JSON.stringify({ type: "blobUrl", blobUrl: blobUrl, filename: filename }));

      // Send the Blob URL to the React Native environment
      // console.log('window***',window.ReactNativeWebView.postMessage(blobUrl));
      // window.ReactNativeWebView.postMessage(JSON.stringify({ type: "blobUrl", url: blobUrl, filename: filename }));
      // window.ReactNativeWebView.postMessage(JSON.stringify({ type: "blobUrl", blob: blob, filename: filename }));
